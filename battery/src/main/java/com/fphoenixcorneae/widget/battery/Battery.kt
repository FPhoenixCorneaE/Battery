...
class Battery @JvmOverloads constructor(
    ...
    private fun obtainStyledAttributes(context: Context, attrs: AttributeSet?) {
        ...
        // 默认水平头朝右
        mOrientation = when (typedArray.getInt(R.styleable.Battery_batteryOrientation, 1)) {
        ...
        // 电池内边距
        mBorderPadding = typedArray.getDimensionPixelSize(R.styleable.Battery_batteryBorderPadding, 2.dp).toFloat()
        // 电池边框厚度
        mBorderWidth = typedArray.getDimensionPixelSize(R.styleable.Battery_batteryBorderWidth, 2.dp).toFloat()
        // 电池边框圆角
        mBorderRadius = typedArray.getDimensionPixelSize(R.styleable.Battery_batteryBorderRadius, 2.dp).toFloat()
        // 电池边框颜色
        mBorderColor = typedArray.getColor(R.styleable.Battery_batteryBorderColor, Color.WHITE)
        // 电池实心部分
        mLowPowerColor = typedArray.getColor(R.styleable.Battery_batteryLowPowerColor, Color.RED)
        mLowPowerThreshold = typedArray.getInt(R.styleable.Battery_batteryLowPowerThreshold, 20)
        mMediumPowerColor = typedArray.getColor(R.styleable.Battery_batteryMediumPowerColor, Color.YELLOW)
        mMediumPowerThreshold = typedArray.getInt(R.styleable.Battery_batteryMediumPowerThreshold, 40)
        mHighPowerColor = typedArray.getColor(R.styleable.Battery_batteryHighPowerColor, Color.WHITE)
        mChargingPowerColor = typedArray.getColor(R.styleable.Battery_batteryChargingPowerColor, Color.GREEN)
        mCurrentPower = typedArray.getInt(R.styleable.Battery_batteryPower, 0)
        mChargingSpeed = typedArray.getInt(R.styleable.Battery_batteryChargingSpeed, 1)
        if (mChargingSpeed !in 1..9) {
            mChargingSpeed = 1
        }
        // 电池头
        mHeaderColor = typedArray.getColor(R.styleable.Battery_batteryHeaderColor, Color.WHITE)
        mHeaderWidth = typedArray.getDimensionPixelSize(R.styleable.Battery_batteryHeaderWidth, 2.dp).toFloat()
        typedArray.recycle()
    }
